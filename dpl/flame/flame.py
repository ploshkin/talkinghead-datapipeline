...
from dpl.flame.lbs import lbs, batch_rodrigues, vertices2landmarks
...
class FLAMETex_trainable(nn.Module):
    ...
    def __init__(self, config):
        ...
        MU = torch.from_numpy(np.reshape(texture_model["MU"], (1, -1))).float()[
            None, ...
        ]
        PC = torch.from_numpy(
            np.reshape(texture_model["PC"], (-1, num_tex_pc))[:, :tex_params]
        ).float()[None, ...]
        self.register_buffer("MU", MU)
        self.register_buffer("PC", PC)

        if "specMU" in texture_model.files:
            specMU = torch.from_numpy(
                np.reshape(texture_model["specMU"], (1, -1))
            ).float()[None, ...]
            specPC = torch.from_numpy(
                np.reshape(texture_model["specPC"], (-1, num_tex_pc))
            )[:, :tex_params].float()[None, ...]
    ...
    def forward(self, texcode):
        diff_albedo = (
            self.MU
            + (self.PC * texcode[:, None, :]).sum(-1)
            + (self.PC_correction * texcode[:, None, :]).sum(-1)
        )
        if self.isspec:
            spec_albedo = self.specMU + (self.specPC * texcode[:, None, :]).sum(-1)
            texture = (
                diff_albedo + spec_albedo
            )  # torch.pow(0.6*(diff_albedo + spec_albedo), 1.0/2.2)
        else:
            texture = diff_albedo
        texture = texture.reshape(texcode.shape[0], 512, 512, 3).permute(0, 3, 1, 2)
        texture = F.interpolate(texture, [256, 256])
