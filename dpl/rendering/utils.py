...
def deg2rad(tensor):
    ...
    return tensor * torch.tensor(math.pi).to(tensor.device).type(tensor.dtype) / 180.0
...
def tensor_vis_landmarks(
    ...
    if scale_colors:
        vis_landmarks /= 255.0  # , dtype=torch.float32)
...
def cut_image(filePath, kps, expand_ratio, leftTop, rightBottom):
    ...
    dstImage = np.zeros(
        shape=[rightBottom[1] - leftTop[1], rightBottom[0] - leftTop[0], channels],
        dtype=np.uint8,
    )
    dstImage[:, :, :] = 0
    ...
    dstImage[
        offset[1] : size[1] + offset[1], offset[0] : size[0] + offset[0], :
    ] = originImage[lt[1] : rb[1], lt[0] : rb[0], :]
...
def load_openpose_landmarks(fname):
    ...
    lmks_with_confidence = np.vstack(
        np.split(np.array(lmk_data["people"][0]["face_keypoints_2d"]), 70)
    )
...
def get_image_cut_box(leftTop, rightBottom, ExpandsRatio, Center=None):
    ...
    def _expand_crop_box(lt, rb, scale):
        ...
        # expand it
        lt, rb = np.array([center[0] + xl, center[1] + yt]), np.array(
            [center[0] + xr, center[1] + yb]
        )
        lb, rt = np.array([center[0] + xl, center[1] + yb]), np.array(
            [center[0] + xr, center[1] + yt]
        )
        center = (lt + rb) / 2
    ...
    Center, leftTop, rightTop, rightBottom, leftBottom = _expand_crop_box(
        leftTop, rightBottom, ExpandsRatio
    )
